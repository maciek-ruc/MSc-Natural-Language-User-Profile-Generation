...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "Return only the final profile as plain text. Start directly with 'I' or 'My'. "
    "Do not summarize the evidence, do not mention the prompt or data, and do not use headings, labels, or bullets."
)
...
def _sanitize_profile_text(text: str, min_sentences: int = 3, max_sentences: int = 5) -> str:
    ...
    cleaned = re.sub(
        r"\b(?:Main Topics|Key Topics|Themes|Profile Summary|Research Areas|Top Topics|Top Categories|Evidence from Interactions|Evidence from Publications)\s*:",
        " ",
        cleaned,
        flags=re.IGNORECASE,
    )
    ...
    cleaned = re.sub(
        r"^(?:based on the provided (?:text|data|evidence(?: from [^.]+)?)\s*,?\s*)",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
    cleaned = re.sub(
        r"^(?:to answer your question\s*,?\s*)",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
    cleaned = re.sub(
        r"^(?:here(?: are|'s)\s+(?:the\s+)?(?:main\s+points|summary)\s*:?)\s*",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
    cleaned = re.sub(
        r"^(?:i will (?:attempt to )?(?:identify|summarize|create)\b[^.]*\.\s*)",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
    cleaned = re.sub(
        r"^(?:the provided text\b[^.]*\.\s*)",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
    ...
    while sentences and re.search(
        r"^(?:based on the provided|to answer your question|here(?: are|'s)|i will (?:attempt to )?(?:identify|summarize|create)|the provided text)",
        sentences[0],
        flags=re.IGNORECASE,
    ):
        sentences.pop(0)
    ...
    for sentence in sentences:
        ...
        normalized = re.sub(
            r"^(I am interested in\s+)?the topics covered include\s+",
            "I am interested in ",
            normalized,
            flags=re.IGNORECASE,
        )
        normalized = re.sub(
            r"^(I am interested in\s+)?it appears to be a collection of research papers(?: and articles)? related to\s+",
            "I am interested in ",
            normalized,
            flags=re.IGNORECASE,
        )
        normalized = re.sub(
            r"^(I am interested in\s+)?(?:research topics|research gaps|topic\s*\d+)\s*:\s*",
            "I am interested in ",
            normalized,
            flags=re.IGNORECASE,
        )
        normalized = re.sub(
            r"^(I am interested in\s+)?introduction to\s+",
            "I am interested in ",
            normalized,
            flags=re.IGNORECASE,
        )
        if re.search(
            r"(?:interaction_categories`? list is empty|i couldn't determine any specific categories|possible categories could be|to identify the main themes|to identify the top categories|the text includes titles, authors)",
            normalized,
            flags=re.IGNORECASE,
        ):
            continue
...
def _maybe_rewrite_profile(client: LLMClient, profile_text: str, evidence_json_text: str) -> tuple[str, dict[str, Any]]:
    ...
    rewrite_meta = {
        ...
        "rewrite_selected": False,
        "original_raw": profile_text,
        "original_sanitized": finalized,
        "original_passes_quality": original_passes,
    }
    if original_passes:
        rewrite_meta["final_source"] = "original"
        return finalized, rewrite_meta
    ...
    rewrite_meta["rewritten_raw"] = rewritten_raw
    rewrite_meta["rewritten_sanitized"] = rewritten
    rewritten_passes = bool(rewritten) and _passes_quality_gate(rewritten)
    rewrite_meta["rewritten_passes_quality"] = rewritten_passes
    if rewritten_passes:
        rewrite_meta["rewrite_selected"] = True
        rewrite_meta["rewrite_reason"] = "Rewritten candidate passed the quality gate after the original failed."
        rewrite_meta["final_source"] = "rewritten"
        return rewritten, rewrite_meta

    if not rewritten:
        rewrite_meta["rewrite_failed"] = True
        rewrite_meta["final_source"] = "original"
        return finalized, rewrite_meta

    rewrite_meta["rewrite_reason"] = "Rewritten candidate did not pass the local quality gate; keeping the original for downstream evaluation."
    rewrite_meta["final_source"] = "original"
    return finalized, rewrite_meta
...
def _passes_quality_gate(profile_text: str) -> bool:
    ...
    low = text.lower()
    if "publications and interactions" in low:
        return False
    meta_markers = [
        "i am interested in based on the provided",
        ...
        "it appears to be a collection of research papers",
        "i couldn't determine any specific categories",
        "possible categories could be",
        "to identify the main themes",
        "to identify the top categories",
        "research gaps:",
        "research topics:",
    ]
    if any(marker in low for marker in meta_markers):
        return False
    if re.search(r"(?:^|\s)(?:key topics|themes|top categories|top topics|interacted articles|evidence from publications|research gaps|research topics|topic\s*\d+)\s*:", low):
        return False
    return True
...
def _get_user_interaction_summary(user_id: int, max_examples: int) -> dict[str, Any]:
    ...
    return {
        ...
        "interacted_articles": prompt_interactions,
    }


def _format_interacted_articles(summary: dict[str, Any], max_abstract_chars: int = 350) -> list[str]:
    lines: list[str] = []
    for idx, item in enumerate(summary.get("interacted_articles", []), start=1):
        title = item.get("title") or ""
        abstract = (item.get("abstract") or "").strip()
        if len(abstract) > max_abstract_chars:
            abstract = abstract[:max_abstract_chars] + "..."

        lines.append(f"{idx}. event_time={item.get('event_time')}")
...
def _has_interaction_signal(summary: dict[str, Any]) -> bool:
    return (
        len(summary.get("interacted_articles", [])) > 0
...
def _build_evidence_json(
    ...
    interactions = summary.get("interacted_articles", [])
    return {
        "publications": [
            {
                "title": pub.get("title"),
        ...
        "interaction_categories": summary.get("top_categories", []),
        "interaction_topics": summary.get("top_topics", []),
    }


def _build_textual_evidence_block(
    publications: list[dict[str, Any]],
    summary: dict[str, Any],
    *,
    source_priority_instruction: str | None = None,
    max_publication_abstract_chars: int = 400,
    max_interaction_abstract_chars: int = 350,
) -> str:
    ...
    interaction_lines = [
        ...
        "Interacted articles:",
        *_format_interacted_articles(summary, max_abstract_chars=max_interaction_abstract_chars),
    ]
    publication_lines = _format_publication_entries(
        publications,
        max_abstract_chars=max_publication_abstract_chars,
    )

    sections: list[tuple[str, list[str]]] = [
        ("Evidence from interactions:", interaction_lines),
        ("Evidence from publications:", publication_lines),
    ]
    if priority == "publications":
        sections = [
            ("Evidence from publications (PRIMARY):", publication_lines),
            ("Evidence from interactions (SECONDARY):", interaction_lines),
        ]
    elif priority == "interactions":
        sections = [
            ("Evidence from interactions (PRIMARY):", interaction_lines),
            ("Evidence from publications (SECONDARY):", publication_lines),
        ]
...
def _build_hybrid_prompt(
    ...
    if priority == "publications":
        objective_line = "Primary objective: emphasize publication evidence."
        sentence_plan = "Sentence plan: sentences 1-3 should reflect publication trajectory; sentence 4 may mention interaction recency."
    elif priority == "interactions":
        objective_line = "Primary objective: emphasize interaction evidence."
        sentence_plan = "Sentence plan: sentences 1-3 should reflect interaction behavior; sentence 4 may mention publication background."
    ...
    lines: list[str] = [
        ...
        "The final output must already be the profile itself, not an analysis or summary of the evidence.",
    ...
    if source_priority_instruction:
        lines.append(f"When signals conflict, apply this priority rule: {source_priority_instruction}")
...
def _generate_profile_from_hybrid_evidence(
    ...
    *,
    ...
    meta = {
        ...
        "weighted_interaction_cap": weighted_interaction_cap if weighted_interaction_cap is not None else len(summary.get("interacted_articles", [])),
        "source_priority_instruction": source_priority_instruction,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional JSON file containing a list of user IDs to generate profiles for.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=0,
        help="Optional batch size used to slice the loaded user list (e.g. 10).",
    )
    parser.add_argument(
        "--batch-index",
        type=int,
        default=1,
        help="1-based batch index used with --batch-size.",
    )
    ...
    parser.add_argument(
        "--min-interacted-articles",
        type=int,
        default=1,
        help="Minimum number of concrete interacted articles required to generate a hybrid profile.",
    )
    parser.add_argument(
        "--output",
        type=str,
